<?php
  ...
  $sqlSelectUser = "select * from user_info where UserID='$userID';";
  $resultq = mysqli_query($conn, $sqlSelectUser);

  $name = $uPass = $cPass = $dob = $gender = $email = $phone = $address = $edu = $blood = $userType = "";
  $errExist = 0;
  $regSuccessful = "";
  ...
  if(mysqli_num_rows($resultq) > 0)
  {
    ...
    $name = $row['Name']; $dob = $row['DoB']; $gender = $row['Gender']; $email = $row['Email']; $phone = $row['Phone']; $address = $row['Address']; $edu = $row['Education']; $blood = $row['BloodGroup']; $userType = $row['UserType'];
...
if(empty($_POST['phn']))
  ...
  if($errExist == 0)
  {
    ...
    {
      $age='27';
       $sqlUserInsert = "update user_info set UserID=['$userID'], Password=['$uPasstoDb'], Name=['$name'], DoB=['$dob'], Gender=['$gender'], Email=['$email'], Phone=['$phone'], Address=[$address], Education=['$edu'], BloodGroup=['$blood'], UserType='$userType where UserID=$userID;"
...
<link rel="stylesheet" href="CSS/style.css">
  ...
  <body>
    <div align="center">
      ...
      <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
        ...
        <table>
          ...
          <tr>
            ...
            <td> <input type="text" name="uid" value=" <?php echo $userID ?>" required> <span style="color:red;"><?php echo $userIDErr; ?></span> </td>
          </tr>
          ...
          <tr>
            ...
            <td>+880 <input type="number" name="phn" value="<?php echo $phone ?>" min="1" required> </td>
          </tr>
          <tr>

          <tr>
            <td valign="top"> <label>Address: </label> </td>
            <td> <textarea name="addr" rows="8" cols="80" required><?php echo $address ?></textarea> </td>
          </tr>

          <tr>
            <td valign="top"> <label>Education & Qualification: </label> </td>
            <td> <textarea name="edu" rows="8" cols="80" required><?php echo $address ?></textarea></td>
          </tr>
          ...
          <tr>
            ...
            <td>
              <select name="utype">
                ...
                <option value="Sales & Marketing Manager" <?php if($userType == "Sales & Marketing Manager")  ?>>Sales & Marketing Manager</option>
              </select>
     ...
     <button onclick="window.location.href='promanusermanagement.php';">Back</button>
